...
def Ratio_Q(df_BS, df_IS, year, quarter=None, industry=None):
    """Subset dataset based on input parameters"""
    ...
    df_some = df_all[['公司', '股票代號', '產業', '年份', '季度', '營業收入', '營業成本',
                      '營業毛利', '營業利益', '財務成本', '稅前淨利', '稅後淨利', '每股稅後盈餘(元)',
                      '現金及約當現金', '應收帳款', '存貨', '流動資產合計', '資產總額', '短期借款',
                      '應付帳款', '流動負債合計', '長期借款', '股東權益總額']]
...
def cal_ratio(df, quarter=None):
    ...
    df['Net Debt to Equity Ratio'] = (df['短期借款'] + df['長期借款'] - df['現金及約當現金'])
                                     / df['股東權益總額']
...
def genPLEL(df_BS, df_IS, year, quarter=None, industry=None):
    """Generate financial ratio statements with profitability / Liquidity / Efficiency / Leverage"""
    result = Ratio_Q(df_BS, df_IS, year, quarter, industry)
    df = cal_ratio(result, quarter)

    df_pro = df.loc[:, ['Name', 'Ticker', 'Gross Margin', 'Operating Margin',
                        'Net Margin', 'EPS (NTD)', 'ROE', 'ROA']]
    df_liq = df.loc[:, ['Name', 'Ticker', 'Current Ratio', 'Quick Ratio',
                        'Interest Coverage Ratio']]
    df_eff = df.loc[:, ['Name', 'Ticker', 'DSO', 'DIO', 'DPO', 'CCC']]
    df_lev = df.loc[:, ['Name', 'Ticker', 'Debt to Equity Ratio',
...
def genPivot(df_IS):
    ...
    p1 = pd.pivot_table(data=df_IS, index=['產業', '公司'],
                        columns=['年份', '季度'], values='營業收入')
    p2 = pd.pivot_table(data=df_IS, index=['產業', '公司'],
                        columns=['年份', '季度'], values='稅後淨利')
    return p1, p2


def drawROPlot(df_BS, df_IS, industry='半導體業', stype='ROE'):
    """Draw ROE or ROA plot"""
    if industry == None:
        industry = df_BS['產業'][0]

    df_all = df_IS.merge(df_BS, on=['公司', '股票代號', '產業', '年份', '季度'])
    df_some = df_all[['公司', '股票代號', '產業', '年份', '季度', '營業收入', '營業成本',
                      '營業毛利', '營業利益', '財務成本', '稅前淨利', '稅後淨利',
                      '每股稅後盈餘(元)', '現金及約當現金', '應收帳款', '存貨', '流動資產合計',
                      '資產總額', '短期借款', '應付帳款', '流動負債合計', '長期借款', '股東權益總額']]
    ...
    plt.legend(corps)
    figPath = '%s_%s.png' % (industry, stype)
    plt.savefig(figPath)
    plt.close()
...
def writeResultExcel(outPath, dfs, ps, imgPaths):
    ...
    with pd.ExcelWriter(outPath, engine='xlsxwriter') as writer:
        ...
        p1.to_excel(writer, sheet_name='pivot', startrow=1, startcol=0)
        p2.to_excel(writer, sheet_name='pivot', startrow=10, startcol=0)
    ...
    os.remove(imgPaths[0])
    os.remove(imgPaths[1])
    
    return None


if __name__ == '__main__':
    companies = [2330, 2303, 5347]
...
#    df_BS = pd.read_excel('./df_BS.xlsx')
    ...
    roePath = drawROPlot(df_IS, df_BS, '半導體業', 'ROE')
    roaPath = drawROPlot(df_IS, df_BS, '半導體業', 'ROA')
